...
# 35 DWI participants with no BIDS fmaps; QSIPREP_NO_SYN_SDC=1 (omit --use-syn-sdc).
# See fmaps.md / prior discussion on fieldmap-less SDC tradeoffs.
#
#   ./submit_dwi_35.sh
#
# List: subject_list_rerun_dwi_no_fmap_no_syn_35.txt
# Throttle: ARRAY_CONCURRENCY=5  ->  Slurm  --array=1-35%5
# -----------------------------------------------------------------------------
...
SUBJECT_LIST_FILE="${SUBJECT_LIST_FILE:-${PROJECT_ROOT}/subject_list_rerun_dwi_no_fmap_no_syn_35.txt}"
...
echo "submit_dwi_35: ${N} subject(s) | PIPELINE_MODE=all (QSIPrep+QSIrecon) | %${ARRAY_CONCURRENCY} | NO_SYN=1"
echo "RESULTS_ROOT=${RESULTS_ROOT}"
echo "List: ${SUBJECT_LIST_FILE}"

export PIPELINE_MODE=all
export QSIPREP_FMAP_RETRY=0
export QSIPREP_NO_SYN_SDC=1
...
exec sbatch \
  --job-name=dwi_35 \
  --output="${PROJECT_ROOT}/logs/dwi_35_%A_%a.out" \
  --error="${PROJECT_ROOT}/logs/dwi_35_%A_%a.err" \
