#!/bin/bash
# Slurm array: FastSurfer recon + SD_STREAM + full connectome + nodestrength.
# Launched by scripts/submit_urmc_g1_recon_backfill.sh

#SBATCH --job-name=urmc_g1_recon
#SBATCH --output=logs/urmc_g1_recon_%A_%a.out
#SBATCH --error=logs/urmc_g1_recon_%A_%a.err
...
: "${DWI_ROOT:?ERROR [array_urmc_g1_recon_backfill]: DWI_ROOT not set}"
: "${REPO_ROOT:?ERROR [array_urmc_g1_recon_backfill]: REPO_ROOT not set}"
...
echo "=== URMC G1 recon+backfill task ${SLURM_ARRAY_TASK_ID}: sub-${SUBJECT} ==="
echo "Step 2: recon"
