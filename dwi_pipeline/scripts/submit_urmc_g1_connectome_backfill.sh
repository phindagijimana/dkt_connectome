#!/bin/bash
# Submit Group 1 connectome + SD_STREAM backfill (local RESULTS_ROOT).
#
# Usage:
#   bash dwi_pipeline/scripts/submit_urmc_g1_connectome_backfill.sh
...
# Always use the G1 connectome backfill list (ignore stale SUBJECT_LIST_FILE in env).
export SUBJECT_LIST_FILE="${URMC_G1_CONN_LIST:-${DWI_ROOT}/subject_list_urmc_g1_connectome_backfill.txt}"
export SUBJECT_LIST_USE_EXISTING=1
export ARRAY_SCRIPT="${DWI_ROOT}/scripts/array_urmc_g1_connectome_backfill.sh"
