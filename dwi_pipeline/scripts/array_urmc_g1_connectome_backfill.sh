#!/bin/bash
# Slurm array: SD_STREAM + full iFOD2 connectome + nodestrength (Group 1, aparc present).
# Launched by scripts/submit_urmc_g1_connectome_backfill.sh

#SBATCH --job-name=urmc_g1_conn
#SBATCH --output=logs/urmc_g1_conn_%A_%a.out
#SBATCH --error=logs/urmc_g1_conn_%A_%a.err
...
: "${DWI_ROOT:?ERROR [array_urmc_g1_connectome_backfill]: DWI_ROOT not set}"
: "${REPO_ROOT:?ERROR [array_urmc_g1_connectome_backfill]: REPO_ROOT not set}"
...
echo "=== URMC G1 connectome backfill task ${SLURM_ARRAY_TASK_ID}: sub-${SUBJECT} ==="
echo "Step 3b: sdstream"
bash "${PIPELINE}" sdstream "${SUBJECT}" "${SUBJECT_ARGS[@]}"
