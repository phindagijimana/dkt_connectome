#!/usr/bin/env bash
# Push dkt_vbt.sif to Docker Hub as an OCI image (Apptainer oras transport).
#
# Prerequisites:
#   - apptainer
#   - Existing dkt_vbt.sif (build first: bash build_vbt.sh)
...
#
# Optional:
#   IMAGE_TAG=0.1.0 SIF=/path/to/dkt_vbt.sif bash publish_dockerhub.sh
#
set -euo pipefail
...
IMAGE_NAME="${IMAGE_NAME:-dkt-vbt}"
IMAGE_TAG="${IMAGE_TAG:-0.1.0}"
SIF="${SIF:-/path/to/others/containers/dkt_vbt.sif}"
